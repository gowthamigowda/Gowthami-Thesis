document.addEventListener("DOMContentLoaded", function () {
    ...
    let root = { name: "Root", type: "Root", children: [], color: "inherit" };
...
document.getElementById("addInnerNode").addEventListener("click", function () {
    ...
    if (currentParent.data.type === "Leaf") {
        ...
        // Ensure newNodeName is not empty and the user didn't click Cancel
        if (newNodeName !== null && newNodeName.trim() !== "") {
            // Create a new inner node with the given name
            const newInnerNode = { name: newNodeName, children: [] };
...
document.getElementById("addLeafNode").addEventListener("click", function () {
    ...
    if (userChoice === "1") {
        ...
        if (newNodeName !== null && newNodeName.trim() !== "") {
            ...
            const newLeafNode = { name: newNodeName, type: "Leaf", children: [] };
...
function loadIndicatorsFromFile(file) {
    ...
    reader.onload = function (event) {
        try {
            ...
            root = { name: "Root", type: "Root", children: [], color: "inherit" };
            rootHierarchy = d3.hierarchy(root);
